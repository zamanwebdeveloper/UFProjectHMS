...
function PaymentMadeController($scope, $http) {
    ...
    $scope.amountReceived = function () {
        ...
        {
            if(parseFloat(received_amount) > parseFloat($scope.bills[i].due_amount))
            {
                $scope.bill_amount[i] = $scope.bills[i].due_amount;
                received_amount = parseFloat(received_amount) - parseFloat($scope.bills[i].due_amount);
            }
            else
            {
    ...
    $scope.calculateExcessPayment = function (index) {
        ...
        for(var i = 0; i < $scope.bills.length; i++)
        {
            ...
            if(parseFloat($scope.bill_amount[i]) > parseFloat($scope.bills[i].due_amount))
            {
                $scope.bill_amount[i] = $scope.bills[i].due_amount;
            }
